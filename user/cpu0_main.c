...
int core0_main(void)
{
    ...
    adc_init(ADC_CHANNEL, ADC_12BIT);

    Sliding_Filter_Init(&vpp_filter, 16);  // 窗口大小为16
    LowPass_Filter_Init(&ADC_LowPass_filter, 0, 0); //
    zf_assert(vpp_filter.is_initialized == 1); // 确保初始化成功

    pwm_init(ATOM0_CH3_P22_2, 10000, 5000);
