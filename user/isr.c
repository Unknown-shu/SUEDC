...
//     Test = adc_mean_filter_convert(ADC0_CH0_A0, 5);        // 采集5次 然后返回平均值
    if(send_flag == 0)
    {

        static uint16 temp = 0, last_temp = 0, next_flag = 0;

        temp = adc_convert(ADC_CHANNEL);         // 采集一次
//        if(temp < 10 && last_temp > 20)
//        {
            next_flag = 1;
//            temp = 0;
//            last_temp = 0;
...
//        last_temp = temp;
        ...
        if(next_flag == 1)
        {
            adc_sample_buffer[adc_sample_index] = temp;         // 获取平均值
    //        adc_sample_buffer[adc_sample_index] = Sliding_Filter_GetAverage(&ADC_filter);         // 获取平均值
            adc_sample_index++;
            if(adc_sample_index >= ADC_SAMPLE_SIZE)
            {
                adc_sample_index = 0;
//                send_flag = 1;                      // 采集完成
                next_flag = 0;
